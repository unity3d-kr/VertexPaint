...
namespace JBooth.VertexPainterPro
{
	...
	public class BakeAO : IVertexPainterUtility
	{
		...
		List<Component> AddColliders()
		{
			List<Component> list = new List<Component>();

			foreach (GameObject go in aoColliderList)
			{
				foreach (MeshRenderer mr in go.GetComponentsInChildren<MeshRenderer>())
				{
					if (mr.gameObject.GetComponent(typeof(Collider)) == null)
					{
						var c = mr.gameObject.AddComponent(typeof(MeshCollider));
						list.Add(c);
					}
		...
		void ClearColliders(List<Component> list)
		{
			if (list != null)
			{
			...
			for (int jIdx = 0; jIdx < jobs.Length; ++jIdx)
			{
				...
				for (int i = 0; i < verts.Length; i++)
				{
					...
					for (int j = 0; j < aoSamples; j++)
					{
						...
						if (Physics.Linecast(v - (offset * 0.1f), v + ray, out hit, ~0))
						{
					...
					if (aoLights != null && aoLights.Length > 0)
					{
						...
						if (aoOneMinusColor)
						{
							window.brushColor = Color.white - window.brushColor;
							window.floatBrushValue = 1 - window.floatBrushValue;
						}
