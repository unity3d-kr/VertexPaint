...
namespace JBooth.VertexPainterPro
{
	public partial class VertexPainterWindow : EditorWindow
	{
		...
		public BrushValueMode brushValueMode = BrushValueMode.Normal;
		...
		public BrushVisualization brushVisualization = BrushVisualization.Disk;
		...
		void PaintMesh( PaintJob j, Vector3 point, Lerper lerper, object value )
		{
			...
			else 
			{
				...
				affected = indices.Count > 0;
				for( int i = 0; i < indices.Count; ++i )
				{
					lerper.Invoke( j, indices[i], ref value, strs[i] );
				}
			}
			if( affected )
			{
		...
		private object MeanValue( PaintJob j, List<int> indices, List<float> strs)
		{
			var cs = j.stream.colors;
			float weight = 0;
